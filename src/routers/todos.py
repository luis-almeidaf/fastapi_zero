...
T_Todo_Filter = Annotated[FilterTodo, Query()]


@router.post("/", status_code=HTTPStatus.CREATED, response_model=TodoPublic)
async def create_todo(todo: TodoSchema, user: CurrentUser, session: T_Session):
    todo = Todo(
        title=todo.title,
...
async def list_todos(
    session: T_Session, user: CurrentUser, todo_filter: T_Todo_Filter
):
